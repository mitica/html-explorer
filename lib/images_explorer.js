...
internal.explore = function(page, options) {
	...
	return Promise.map(images, function(image) {
		return internal.identifyImage(image, options)
			.then(function() {
...
internal.createPageImages = function(page, images, options, imageFilter) {
	...
	// ordering images:
	function ratingSort(image) {
		return parseInt(image.rating / 3);
	}

	function widthSort(image) {
		return image.width || 0;
	}

	function viewWidthSort(image) {
		return image.viewWidth || image.width || 0;
	}
	page.images = _.sortBy(page.images, [ratingSort, widthSort, viewWidthSort, 'rating']);
	page.images.reverse();
	// limit images:
...
internal.getWords = function(text) {
	return text.split(/[\s\.\:\?\!\(\)'"`„”,;_-]/g).filter(function(word) {
