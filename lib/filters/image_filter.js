...
var Filter = module.exports = function Filter(options) {
  ...
  if (_.isString(options.invalidTypes))
...
Filter.prototype.filter = function(image) {
  ...
  if (image.type && _.isArray(options.invalidTypes) && options.invalidTypes.indexOf(image.type) > -1) return false;
  if (image.type && _.isArray(options.types) && options.types.indexOf(image.type) < 0) return false;
...
function passSizeFilter(image, widthName, heightName, options) {
  ...
  if (hasSize && height === width && height < 401) return false;
