...
Filter.prototype.filter = function(target) {
	...
	if (target.sourceType && utils.VIDEO_TYPES.indexOf(target.sourceType) < 0) {
		return false;
	}
	...
	if (target.src && options.src.test(target.src)) {
		return false;
	}
	if (target.src && options.extraSrc && options.extraSrc.test(target.src)) {
		return false;
	}
