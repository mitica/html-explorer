...
internal.getPage = function(url, options) {
  ...
  return utils.request(reqOptions).then(function(result) {
    ...
    result.body = internal.decodePage(result.response, result.body);
    options.html && (page.html = result.body);
    page.dom = cheerio.load(result.body);
    return page;
  });
};

internal.decodePage = function(response, buffer) {
  var encoding = charset(response.headers['content-type']) || 'utf8';
...
internal.exploreInfo = function(page) {
  ...
  if (page.title) page.title = page.title.replace(/[\t\r\n]/g, ' ').replace(/ {2,}/g, ' ').trim();
  if (page.description) page.description = page.description.replace(/[\t\r\n]/g, ' ').replace(/ {2,}/g, ' ').trim();
};
