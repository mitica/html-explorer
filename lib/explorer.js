...
var contentExplorer = require('./content_explorer');
	...
	return pageExplorer.explore(url, options.page).then(function(page) {
		return internal.getVideos(page, options.video).then(function() {
			return internal.getImages(page, options.images).then(function() {
				return internal.getContent(page, options.content).then(function() {
					delete page.dom;
...
internal.getContent = function(page, options) {
	if (options === false) {
		return Promise.resolve();
	}

	return contentExplorer.explore(page, options)
		.catch(function() {});
};
